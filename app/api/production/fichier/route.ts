...
export async function GET(req: Request) {
  ...
  if (!HOTES.some(h => h.test(hote)) || !cible.startsWith("https://")) return NextResponse.json({ erreur: "Hôte non relayé." }, { status: 403 });
  const r = await fetch(cible, { redirect: "manual" });
